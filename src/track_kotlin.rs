//! The Kotlin track. **Not live.** Registered, unbuilt.
//!
...
//! answers `false` for every extension [`Lang::Kotlin`] claims, so no scan
//! reads `.kt` and `.kts`. Bringing Kotlin up is `scan: None` becoming
...
/// Kotlin's registration. Not live, so the track owns no file and contributes
