...
pub const HELP: &str = concat!(
    ...
    "     the environment — a config that will not parse, a root that is not\n",
    "     there, a store another scan is holding open for writing — and those\n",
    "     are the ones worth retrying. `gate` also answers 2 when a\n",
    "     comparison could not be made at all: the baseline's or the run's\n",
    "     resolved + unresolved is zero, so there is no rate on that side and\n",
    "     the result is neither a pass nor a regression. That one is\n",
    "     deterministic and a retry answers 2 again. `scan` has no verdict to\n",
    "     fail and so never returns 1; every failure it can have is a 2.\n",
    "\n",
    "scan\n",
