...
//! rate.** So this track emits one reference kind —
//! [`crate::model::RefKind::Import`], the `using` directive — and no call
//! site, no type use and no supertype. That is a deliberate refusal rather
//! than an unfinished job: a tier-2 language that emitted them un-gated would
//! put references in a denominator no tier-2 resolver links, which is tier-1
