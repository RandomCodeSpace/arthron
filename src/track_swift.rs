//! The Swift track. **Not live.** Registered, unbuilt.
//!
...
//! answers `false` for every extension [`Lang::Swift`] claims, so no scan
//! reads `.swift`. Bringing Swift up is `scan: None` becoming
...
/// Swift's registration. Not live, so the track owns no file and contributes
