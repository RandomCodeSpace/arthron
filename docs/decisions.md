...
**Decided: cargo-dist 0.32.0 generates and drives the release workflow.** It
builds the five requested native targets: `x86_64-unknown-linux-gnu`,
`aarch64-unknown-linux-gnu`, `x86_64-apple-darwin`,
`aarch64-apple-darwin`, and `x86_64-pc-windows-msvc`. The generated plan names
an archive and a checksum for each target, plus a unified `sha256.sum` and a
source archive. A `push` matching `v*` is the only publishing event: its host
job receives `contents: write`, uploads every generated asset, and creates the
GitHub Release. `workflow_dispatch` has no publishing condition; it runs
`dist plan` and the full build matrix with root `contents: read`, so it cannot
create a tag, upload a release asset, or create a release.
...
dispatch `Release` on the merged default-branch commit, verify the five
target archives/checksums are uploaded as workflow artifacts and that no
GitHub Release exists; then, only for the planned release tag, verify the
same five target assets and checksums are attached to its GitHub Release.
...
dependency. The generated CI uses `actions/checkout@v6`,
`actions/upload-artifact@v7`, and `actions/download-artifact@v8`, exactly as
generated by cargo-dist 0.32.0; build-time downloads are confined to CI.

The workflow deliberately carries `allow-dirty = ["ci"]`: cargo-dist's
generated workflow is customized only to restrict release tags to `v*`, make
manual dispatch non-publishing, and narrow write permission to the host job.
When updating cargo-dist, regenerate its CI output and review the complete
diff before changing that pin.
